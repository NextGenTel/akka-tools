...
object NgtPersistentActor {
  val DEFAULT_IDLE_TIMEOUT_IN_SECONDS = FiniteDuration(240, TimeUnit.SECONDS)
}

abstract class NgtPersistentActor[E:ClassTag, Ex <: Exception : ClassTag]
(
  idleTimeout:FiniteDuration = NgtPersistentActor.DEFAULT_IDLE_TIMEOUT_IN_SECONDS
  )
  extends Actor
  ...
  private def startTimeoutTimer(): Unit = {
    cancelTimeoutTimer()
    timeoutTimer = Some(context.system.scheduler.scheduleOnce(idleTimeout, self, NgtPersistentActorTimeout()))
  }
...
//  }
  ...
  private def onNgtDurableMessageReceived(msg: DurableMessageReceived) {
    log.debug("Remembering NgtDurableMessageReceived with DeliveryId={}", msg.deliveryId)
  ...
  override def receiveCommand: Receive = {
    case timeout:NgtPersistentActorTimeout =>
      log.debug("Stopping due to inactivity")
      onInactiveTimeout()
    case r:DurableMessageReceived =>
      cancelTimeoutTimer()
      persist(r) {
        e => onNgtDurableMessageReceived(e)
      }
      startTimeoutTimer()
    case cmd:AnyRef =>
...
case class NgtPersistentActorTimeout private [persistence] ()


abstract class NgtPersistentJavaActor[Ex <: Exception : ClassTag](idleTimeout:FiniteDuration) extends NgtPersistentActor[AnyRef, Ex](idleTimeout) with NgtPersistentJavaActorLike {

  def this() = this(NgtPersistentActor.DEFAULT_IDLE_TIMEOUT_IN_SECONDS)

}

trait NgtPersistentJavaActorLike {
...
abstract class NgtPersistentView[E:ClassTag, S:ClassTag](persistenceIdBase:String, id:String, collectHistory:Boolean = true) extends PersistentView with ActorLogging {
