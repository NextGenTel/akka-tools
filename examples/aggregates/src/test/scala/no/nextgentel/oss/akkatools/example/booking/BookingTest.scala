...
class BookingTest(_system:ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with BeforeAndAfter {
  ...
  before {
    ...
    printShop = TestProbe()
    cinema = TestProbe()
    main = system.actorOf(BookingAggregate.props(ourDispatcher.ref.path, DurableMessageForwardAndConfirm(printShop.ref).path, DurableMessageForwardAndConfirm(cinema.ref).path, seatIdGenerator), "BookingAggregate-" + id)
