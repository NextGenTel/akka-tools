...
class StorageRepoMySQLImpl(sql2o: Sql2o, config:StorageRepoConfig, _errorHandler:Option[JdbcJournalErrorHandler]) extends StorageRepoWithClusterNodeRepo {
  ...
  def insertPersistentReprList(dtoList: Seq[JournalEntryDto]) {
    ...
    val sql = s"insert into ${tableName_journal} (typePath, id, sequenceNr, persistentRepr, payload_write_only, updated) " +
      s"values (:typePath, :id, :sequenceNr, :persistentRepr, :payload_write_only, sysdate)"
