...
object JdbcJournal {
  ...
  val NgtJsonSerializer_className = "no.ngt.oss.akkatools.serializing.NgtJsonSerializer"
...
class JdbcSyncWriteJournal extends SyncWriteJournal with ActorLogging {
  ...
  def tryToExtractPayloadAsJson(p: PersistentRepr): Option[String] = {
    // If we can use the no.ngt.oss.akkatools.serializing.NgtJsonSerializer on the payload,
    ...
    if (NgtJsonSerializer_className == serializer.getClass.getName) {
      // we can do it
