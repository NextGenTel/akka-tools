...
(
  ...
  def withSeedNodeList(newSeedNodeList:List[String]):NgtClusterConfig = {
    new NgtClusterConfig(Some(thisHostname), port, newSeedNodeList)
  }
}
